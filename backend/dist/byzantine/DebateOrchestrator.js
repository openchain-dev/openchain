...
exports.debateGenerator = exports.orchestrator = exports.DebateOrchestrator = exports.DEBATE_TOPICS = void 0;
exports.createDebateStream = createDebateStream;
exports.startRound = startRound;
exports.runDebateTurn = runDebateTurn;
exports.runDiscussionPhase = runDiscussionPhase;
exports.runVotingPhase = runVotingPhase;
exports.runFullRound = runFullRound;
exports.getCurrentRound = getCurrentRound;
exports.getRoundHistory = getRoundHistory;
exports.getCoalitionAnalysis = getCoalitionAnalysis;
exports.getDebateTopics = getDebateTopics;
const ByzantineSystem_1 = require("./ByzantineSystem");
// ============================================================================
// DEBATE TOPICS
// ============================================================================
exports.DEBATE_TOPICS = [
    {
        id: 'cip-02',
...
function getRandomTopic() {
    return exports.DEBATE_TOPICS[Math.floor(Math.random() * exports.DEBATE_TOPICS.length)];
}
// ============================================================================
...
class DebateOrchestrator {
    ...
    // Start a New Round
    // --------------------------------------------------------------------------
    async startRound(topicId) {
        const topic = topicId
            ? exports.DEBATE_TOPICS.find(t => t.id === topicId) || getRandomTopic()
            : getRandomTopic();
        ...
        for (const validator of validators) {
            if (validator.byzantineMode === 'sleeper' && !validator.isActivated) {
                if ((0, ByzantineSystem_1.checkSleeperTrigger)(validator, this.stateManager.getNetworkState())) {
    ...
    async runDebateTurn(speakerId) {
        ...
        const statement = await (0, ByzantineSystem_1.generateDebateStatement)(speaker, context);
        this.currentRound.statements.push(statement);
        this.stateManager.addStatement(statement);
        const byzantineEvents = [];
        const detections = [];
        // If speaker is Byzantine, potentially generate Byzantine action
        if (speaker.byzantineMode !== 'honest') {
            const action = await (0, ByzantineSystem_1.generateByzantineAction)(speaker, networkState, this.currentRound.statements);
            if (action) {
                ...
                const honestValidators = validators.filter(v => v.byzantineMode === 'honest' && v.id !== speaker.id);
                for (const detector of honestValidators) {
                    const detectionResult = await (0, ByzantineSystem_1.attemptDetection)(detector, action, this.currentRound.statements.filter(s => s.validatorId === speaker.id), speaker.votingHistory);
    ...
    // --------------------------------------------------------------------------
    async runDiscussionPhase(turnsPerValidator = 2) {
        if (!this.currentRound) {
            throw new Error('No active debate round. Call startRound() first.');
        }
        const validators = this.stateManager.getAllValidators();
        const allStatements = [];
        const allByzantineEvents = [];
        const allDetections = [];
    ...
    async runVotingPhase() {
        ...
        for (const validator of validators) {
            const voteRecord = await (0, ByzantineSystem_1.generateVote)(validator, this.currentRound.topic, networkState, this.currentRound.statements);
...
// ============================================================================
function createDebateStream(orchestrator) {
    return async function* debateGenerator(topicId) {
        try {
            // Start the round
...
exports.orchestrator = new DebateOrchestrator(ByzantineSystem_1.byzantineState);
// Create the stream generator bound to the singleton
exports.debateGenerator = createDebateStream(exports.orchestrator);
// Convenience functions that use the singleton
function startRound(topicId) {
    return exports.orchestrator.startRound(topicId);
}
function runDebateTurn(speakerId) {
    return exports.orchestrator.runDebateTurn(speakerId);
}
function runDiscussionPhase(turnsPerValidator) {
    return exports.orchestrator.runDiscussionPhase(turnsPerValidator);
}
function runVotingPhase() {
    return exports.orchestrator.runVotingPhase();
}
function runFullRound(topicId) {
    return exports.orchestrator.runFullRound(topicId);
}
function getCurrentRound() {
    return exports.orchestrator.getCurrentRound();
}
function getRoundHistory() {
    return exports.orchestrator.getRoundHistory();
}
function getCoalitionAnalysis() {
    return exports.orchestrator.getCoalitionAnalysis();
}
function getDebateTopics() {
    return [...exports.DEBATE_TOPICS];
}
