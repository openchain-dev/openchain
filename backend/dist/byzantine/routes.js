...
const DebateOrchestrator_1 = require("./DebateOrchestrator");
// ============================================================================
// INITIALIZATION
// ============================================================================
const router = (0, express_1.Router)();
const stateManager = ByzantineSystem_1.byzantineState;
...
router.get('/stream', async (req, res) => {
    ...
    try {
        // Create generator
        const debateGen = (0, DebateOrchestrator_1.createDebateStream)(DebateOrchestrator_1.orchestrator);
 ...
 */
router.get('/continuous', async (req, res) => {
    const surveillanceMode = req.query.surveillance === 'true';
    ...
    while (running && !res.destroyed) {
        try {
            roundCount++;
            // Pick random topic
            const randomTopic = DebateOrchestrator_1.DEBATE_TOPICS[Math.floor(Math.random() * DebateOrchestrator_1.DEBATE_TOPICS.length)];
            console.log(`[BYZANTINE] Starting continuous round ${roundCount}: ${randomTopic.title}`);
            // Stream full debate
            const debateGen = (0, DebateOrchestrator_1.createDebateStream)(DebateOrchestrator_1.orchestrator);
            const generator = debateGen(randomTopic.id);
            for await (const event of generator) {
                if (!running || res.destroyed)
                    break;
                sendEvent(event.type, event.data);
            }
            if (!running || res.destroyed)
 ...
 */
router.get('/validators/surveillance', (req, res) => {
    const validators = stateManager.getAllValidators();
...
router.get('/analysis', (req, res) => {
    ...
    const rounds = (0, DebateOrchestrator_1.getRoundHistory)();
    ...
    res.json({
        coalitionAnalysis: (0, DebateOrchestrator_1.getCoalitionAnalysis)(),
        actorStats,
...
router.get('/topics', (req, res) => {
    res.json({
        topics: DebateOrchestrator_1.DEBATE_TOPICS,
        count: DebateOrchestrator_1.DEBATE_TOPICS.length,
        categories: [...new Set(DebateOrchestrator_1.DEBATE_TOPICS.map(t => t.category))]
    });
});
/**
 * POST /trigger-debate
 ...
 * Body: { topicId?: string }
 */
router.post('/trigger-debate', async (req, res) => {
    const { topicId } = req.body;
    try {
        const result = await DebateOrchestrator_1.orchestrator.runFullRound(topicId);
        res.json({
            success: true,
            round: {
                id: result.round.id,
 ...
 */
router.post('/update-network', (req, res) => {
...
router.get('/round', (req, res) => {
    const round = (0, DebateOrchestrator_1.getCurrentRound)();
    if (!round) {
        return res.json({
            active: false,
...
router.get('/health', (req, res) => {
    ...
    const currentRound = (0, DebateOrchestrator_1.getCurrentRound)();
    const history = (0, DebateOrchestrator_1.getRoundHistory)();
    res.json({
        status: 'healthy',
        timestamp: Date.now(),
