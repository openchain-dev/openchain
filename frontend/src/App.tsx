...
export default function App() {
  ...
  const renderTerminal = () => (
    <div style={{ padding: isMobile ? '24px 16px' : '40px 24px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      {/* Genesis CIP */}
      <div className="cip-card card" style={{ marginBottom: isMobile ? 20 : 32 }}>
        <div style={{ padding: isMobile ? 16 : 20, display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 12, flexWrap: 'wrap' }}>
          <span className="badge badge-outline">CIP-0</span>
          <span className="badge badge-coral">Ratified</span>
          <span className="badge badge-teal" style={{ display: isMobile ? 'none' : 'inline' }}>Foundational</span>
                  <button 
            onClick={() => setExpandedCIP(!expandedCIP)}
                style={{
              marginLeft: 'auto', 
              background: 'none', 
              border: 'none', 
        ...
        <div style={{ padding: isMobile ? '0 16px 16px' : '0 20px 20px' }}>
          <h3 style={{ color: 'var(--coral)', fontSize: isMobile ? 17 : 20, marginBottom: 8 }}>{GENESIS_CIP.title}</h3>
          <p style={{ color: 'var(--text-secondary)', fontSize: isMobile ? 13 : 14, lineHeight: 1.7 }}>{GENESIS_CIP.summary}</p>
          {expandedCIP && (
            <pre className="mono" style={{ 
              ...
              {GENESIS_CIP.fullProposal}
            </pre>
          )}
        </div>
      </div>

      {/* Agent Worker CTA - when sidebar is closed */}
      {!agentPanelOpen && (
        <div 
          ...
          style={{ 
            ...
            background: 'linear-gradient(135deg, rgba(232, 90, 79, 0.1), rgba(78, 205, 196, 0.05))',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ 
                ...
                Click to watch Claw work autonomously in real-time
              </p>
            </div>
          </div>
        </div>
