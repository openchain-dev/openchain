...
export class Block {
  ...
  private static MAX_BLOCK_SIZE = 1024 * 1024; // 1 MB
  ...
  constructor(
    ...
    if (blockSize > Block.MAX_BLOCK_SIZE) {
      throw new Error(`Block size ${blockSize} bytes exceeds the limit of ${Block.MAX_BLOCK_SIZE} bytes`);
    }
  ...
  private calculateBlockSize(): number {
    ...
    const headerSize = Buffer.from(JSON.stringify(this.header)).length;
    const transactionsSize = this.transactions.reduce((total, tx) => {
      return total + Buffer.from(JSON.stringify(tx)).length;
    }, 0);
