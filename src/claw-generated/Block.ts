...
export class Block {
  ...
  public async isValid(previousBlock?: Block): Promise<boolean> {
    ...
    const validTransactions = await Promise.all(this.transactions.map(tx => TransactionValidator.validate(tx)));
    return validTransactions.every(valid => valid);
  }
