...
describe('sendTransaction', () => {
  ...
  it('should return an error for a missing signedTransaction parameter', async () => {
    const response = await request(app)
      .post('/transactions')
      .send({})
      .expect(400);
  ...
  it('should return an error for an invalid base64 signedTransaction', async () => {
    const response = await request(app)
      .post('/transactions')
  ...
  it('should return an error for a transaction with invalid signatures', async () => {
    const transaction = new Transaction();
    transaction.add({ keys: [], programId: Buffer.alloc(32), data: Buffer.alloc(100) });
